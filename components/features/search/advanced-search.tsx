  ...
  const getFilteredModules = useCallback(() => {
    ...
    if (filters.rootMethods.length > 0) {
      filtered = filtered.filter((module) =>
        filters.rootMethods.some((method) =>
          module.compatibility.rootMethods.includes(
            method as 'Magisk' | 'KernelSU' | 'KernelSU-Next',
          ),
        ),
      );
    }
    ...
    if (filters.androidVersions.length > 0) {
      filtered = filtered.filter((module) =>
        filters.androidVersions.some((version) =>
          module.compatibility.androidVersions.includes(version),
        ),
      );
    }
    ...
    if (filters.hasSourceUrl !== null) {
      filtered = filtered.filter((module) =>
        filters.hasSourceUrl
          ? module.sourceUrl && module.sourceUrl.length > 0
          : !module.sourceUrl || module.sourceUrl.length === 0,
      );
    }
    ...
    if (filters.hasCommunityUrl !== null) {
      filtered = filtered.filter((module) =>
        filters.hasCommunityUrl
          ? module.communityUrl && module.communityUrl.length > 0
          : !module.communityUrl || module.communityUrl.length === 0,
      );
    }
      ...
      >
        {isMobile && (
          <div className="flex items-center justify-between p-4 border-b">
            ...
            <Button variant="ghost" size="sm" onClick={() => setShowFilters(false)} className="p-2">
              <X className="w-4 h-4" />
            </Button>
          </div>
        )}
            ...
            >
              <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto">
                {categories.map((category) => (
                  <div
                    key={category}
                    className="flex items-center space-x-2 min-h-[2.5rem] px-2 rounded hover:bg-muted/50"
                  >
                    <Checkbox
                      id={category}
            ...
            >
              <div className="grid grid-cols-2 gap-1 max-h-48 overflow-y-auto">
                {androidVersions.map((version) => (
                  <div
                    key={version}
                    className="flex items-center space-x-2 min-h-[2.25rem] px-2 rounded hover:bg-muted/50"
                  >
                    <Checkbox
                      id={version}
