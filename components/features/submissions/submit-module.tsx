...
export function SubmitModule({ userId }: SubmitModuleProps) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      console.log(
        '[Submit] Sending turnstile token:',
        turnstileToken ? `${turnstileToken.substring(0, 20)}...` : 'none',
      );
            ...
            .map((e: { field: string; message: string }) => {
              ...
              if (message.includes('select a valid')) {
                if (e.field.includes('license')) {
                  message =
                    'Please select a license from the dropdown. Open source modules require a valid license.';
        ...
        <Card className="max-w-xl">
          ...
          <CardContent className="space-y-4">
            <Alert>
              ...
              <AlertDescription>
                To submit modules with automatic GitHub release syncing, you&apos;ll need to set up
                a GitHub Personal Access Token (PAT) in your settings.
                <br />
                <br />
                This allows the system to automatically fetch new releases they are published on
                GitHub.
              </AlertDescription>
            </Alert>
