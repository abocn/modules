...
export function ModuleManagement({
  ...
  const getReviewStatusBadge = (module: Module) => {
    ...
    const changesNote = hasReviewNotes
      ? reviewNotes.find((n) => n.type === 'changes-requested')
      : null;
  ...
  const handleToggleFeatured = (id: string) => {
    ...
    if (onToggleFeatured) {
      ...
      setLocalModules(
        localModules.map((m) => (m.id === id ? { ...m, isFeatured: !m.isFeatured } : m)),
      );
    }
                ...
                <TableRow key={module.id}>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded overflow-hidden flex items-center justify-center bg-gradient-to-br from-gray-700 to-gray-900">
                        ...
                        <div
                          className={`w-full h-full flex items-center justify-center text-xs text-white ${module.icon ? 'hidden' : ''}`}
                        >
                  ...
                  <TableCell>
                    <Badge variant="outline">
                      {MODULE_CATEGORIES.find((cat) => cat.id === module.category)?.shortLabel ||
                        module.category}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <Eye
                          className={`w-4 h-4 ${module.isPublished ? 'text-green-600' : 'text-gray-400'}`}
                        />
                      </Button>
                      <Button
                        variant="ghost"
                      ...
                      >
                        <Star
                          className={`w-4 h-4 ${module.isFeatured ? 'fill-yellow-400 text-yellow-400' : ''}`}
                        />
                      </Button>
                      <Button
                        variant="ghost"
