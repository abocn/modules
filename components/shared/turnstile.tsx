...
const loadTurnstileScript = (): Promise<void> => {
  ...
  scriptLoadPromise = new Promise((resolve, reject) => {
    ...
    script.onerror = (event) => {
      ...
      const errorMsg =
        event instanceof ErrorEvent ? event.message : 'Failed to load Turnstile script';
  ...
  (
    ...
    const renderWidget = useCallback(async () => {
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Failed to render Turnstile widget';
        ...
        try {
          ...
          if (mountedRef.current) {
            handleTurnstileError(
              error instanceof Error ? error : new Error('Failed to initialize Turnstile'),
            );
          }
        }
      };
    ...
    const widgetContent = (
      <>
        {isLoading && !renderError && (
          <Skeleton
            className={`${
              size === 'compact' ? 'h-[140px] w-[150px]' : 'h-[65px] w-[300px]'
            } rounded-md border`}
          />
        )}
        <div
    ...
    if (wrapInCard) {
      return (
        <Card className="max-w-md py-4 px-0">
          ...
          <CardContent className="-mt-4">{widgetContent}</CardContent>
        </Card>
      );
    }
