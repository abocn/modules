    ...
    const fetchTrendingModules = async () => {
      ...
      try {
        const response = await fetch(
          `/api/modules?filter=trending&algorithm=${algorithm}&range=${timeRange}`,
        );
        if (!response.ok) {
      ...
      >
        <div className="container mx-auto p-6">
          <Alert className="border-destructive">
            <AlertDescription>Failed to load trending modules: {error}</AlertDescription>
          </Alert>
        </div>
      </SharedLayout>
    );
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <div className="flex gap-2">
              <Select
                value={algorithm}
                onValueChange={(value) => setAlgorithm(value as TrendingAlgorithm)}
              >
