...
export function useAdminModules(): UseAdminModulesReturn {
  ...
  const updateRecommendedStatus = async (
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to update recommended status';
      setError(errorMessage);
      return false;
    } finally {
      setIsLoading(false);
  ...
  const updateModuleWarnings = async (
    moduleId: string,
    warnings: { type: 'malware' | 'closed-source' | 'stolen-code'; message: string }[],
  ): Promise<boolean> => {
    setIsLoading(true);
...
export function useAdminModulesList(
  search?: string,
  limit: number = 20,
  offset: number = 0,
): UseAdminModulesListReturn {
