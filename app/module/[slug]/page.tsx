...
export default async function ModulePage({ params, searchParams }: ModulePageProps) {
  ...
  const getLatestRelease = () => {
    ...
    const sortedReleases = [...displayModuleData.releases].sort(
            ...
            <div className="p-4 sm:p-6 md:p-4 lg:p-6 xl:p-8 space-y-6 sm:space-y-8 w-full max-w-full overflow-x-hidden">
              <div className="flex items-start gap-4 sm:gap-6 lg:gap-8 w-full overflow-hidden">
                ...
                <div className="flex-1 min-w-0 overflow-hidden items-center">
                  <div>
                    <h1 className="text-xl sm:text-2xl font-bold mb-1 break-words leading-tight">
                      {displayModuleData.name}
                    </h1>
                    <div className="flex items-center gap-2">
                      <User className="w-4 h-4 sm:w-5 sm:h-5 text-muted-foreground flex-shrink-0" />
                      <span className="text-muted-foreground text-sm sm:text-base truncate font-medium">
                        {displayModuleData.author}
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6 text-sm sm:text-base mt-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">{displayModuleData.rating}</span>
                    ...
                    <div className="flex items-center gap-2 text-muted-foreground">
                      <span className="font-medium">
                        {displayModuleData.downloads.toLocaleString()}
                      </span>
                      <span>downloads</span>
                    </div>
                  </div>
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3">
                ...
                <Badge variant="outline" className="text-sm px-3 py-1">
                  {displayModuleData.license}
                </Badge>
                {displayModuleData.isFeatured && (
                  <Badge variant="secondary" className="text-sm px-3 py-1">
                    Featured
                  </Badge>
                )}
                {displayModuleData.isRecommended && (
                  <Badge variant="secondary" className="text-sm px-3 py-1">
                    Recommended
                  </Badge>
                )}
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      {displayModuleData.communityUrl && (
                        <Button variant="outline" className="flex-1 bg-transparent h-9" asChild>
                          <a
                            href={displayModuleData.communityUrl}
                            target="_blank"
                      ...
                      {displayModuleData.sourceUrl && (
                        <Button variant="outline" className="flex-1 bg-transparent h-9" asChild>
                          <a
                            href={displayModuleData.sourceUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                          >
                  ...
                  <ul className="space-y-3 -mt-2">
                    {displayModuleData.features.map((feature, index) => (
                      <li key={index} className="flex items-start gap-3 w-full">
                        ...
                        <span className="text-muted-foreground break-words flex-1 text-sm sm:text-base leading-relaxed">
                          {feature}
                        </span>
                      </li>
                    ))}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <span className="font-semibold text-sm text-right">
                        {displayModuleData.lastUpdated}
                      </span>
                    </div>
                    <div className="flex justify-between items-start">
                      <span className="text-muted-foreground text-sm font-medium">Category</span>
                      <span className="font-semibold text-sm text-right">
                        {getCategoryLabel(displayModuleData.category)}
                      </span>
                    </div>
                  </div>
