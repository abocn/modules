  ...
  async (_request, context) => {
    ...
    try {
      ...
      jobExecutionService.executeJob(job.id).catch((error) => {
        console.error(
          `[! /api/admin/release-schedule/manual-run] Failed to execute job ${job.id}:`,
          error,
        );
      });
      ...
      console.error(
        '[! /api/admin/release-schedule/manual-run] Error starting manual sync:',
        error,
      );
      return NextResponse.json({ error: 'Failed to start manual sync' }, { status: 500 });
    }
  },
