...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const categoryCountsQuery = db
      ...
      .from(modules)
      .where(and(eq(modules.isPublished, true), eq(modules.status, 'approved')))
    ...
    const totalStats = {
      ...
      totalModules,
      avgModulesPerCategory:
        categories.length > 0 ? Math.round(totalModules / categories.length) : 0,
      mostPopularCategory:
        categories.length > 0
          ? categories.reduce((prev, current) => (prev.count > current.count ? prev : current)).name
          : null,
    };
