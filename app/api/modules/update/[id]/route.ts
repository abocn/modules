...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (body.status === 'pending' && body.turnstileToken) {
      const turnstileResult = await validateTurnstileToken(body.turnstileToken, {
    ...
    const existingModule = await db
      .select()
      .from(modules)
      .where(and(eq(modules.id, id), eq(modules.submittedBy, session.user.id)))
      .limit(1);
    ...
    if (existingModule[0].isPublished && !body.status) {
      return NextResponse.json({ error: 'Cannot edit published modules' }, { status: 403 });
    }
