...
export async function POST(req: NextRequest) {
  try {
    ...
    const rateLimitResult = await checkRateLimit(user.id, 'module_submit', {
      interval: 60 * 60 * 1000,
      limit: 5,
    });
    ...
    console.log(
      '[API Submit] Received turnstile token:',
      json.turnstileToken ? `${json.turnstileToken.substring(0, 20)}...` : 'none',
    );
    const clientIP = getClientIP(req);
    console.log('[API Submit] Client IP:', clientIP);

    const turnstileResult = await validateTurnstileToken(json.turnstileToken, {
      remoteip: clientIP,
    });
    ...
    if (moduleData.images && moduleData.images.length > 0) {
      ...
      for (let i = 0; i < moduleData.images.length; i++) {
        ...
        console.log(
          `[Image Processing] Downloading screenshot ${i + 1}/${moduleData.images.length}: ${imageUrl}`,
        );
    ...
    console.log(
      `[Module Submitted] ID: ${id}, Name: ${moduleData.name}, User: ${user.id}, Has Release: ${!!releaseData}`,
    );
