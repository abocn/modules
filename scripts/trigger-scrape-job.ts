...
async function triggerScrapeJob() {
  ...
  try {
    ...
    const [completedJob] = await db.select().from(adminJobs).where(eq(adminJobs.id, job.id));
    ...
    const recentActions = await db
      ...
      .where(
        or(
          and(eq(adminActions.targetType, 'system'), eq(adminActions.targetId, `job-${job.id}`)),
          and(eq(adminActions.targetType, 'module'), eq(adminActions.adminId, adminUser.id)),
        ),
      )
