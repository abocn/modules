...
export async function logModuleEdit(adminId: string, moduleInfo: ModuleChangeInfo): Promise<void> {
  const changedFields = Object.keys(moduleInfo.newValues).filter(
    (key) =>
      JSON.stringify(moduleInfo.oldValues[key]) !== JSON.stringify(moduleInfo.newValues[key]),
  );
...
export function compareModuleData(
  oldData: Record<string, unknown>,
  newData: Record<string, unknown>,
): ModuleChangeInfo | null {
...
export async function logUserEdit(adminId: string, userInfo: UserChangeInfo): Promise<void> {
...
export function compareUserData(
  ...
  const relevantFields = ['name', 'email', 'role', 'emailVerified'];
