 ...
 */
export async function transformDbModuleToModule(
  dbModule: DbModule,
...
export async function transformDbModulesToModulesBatch(dbModules: DbModule[]): Promise<Module[]> {
  ...
  const latestReleases = await db
    ...
    .where(
      and(
        sql`${releases.moduleId} IN (${sql.join(
  ...
  const ratingStats = await db
    ...
    .from(ratings)
    .where(
      sql`${ratings.moduleId} IN (${sql.join(
        moduleIds.map((id) => sql`${id}`),
        sql`, `,
      )})`,
    )
...
export async function transformDbModuleToAdminModule(
  dbModule: DbModule,
  includeReleases = false,
): Promise<AdminModule> {
...
export async function getModuleById(id: string, includeReleases = false): Promise<Module | null> {
  const dbModule = await db.select().from(modules).where(eq(modules.id, id)).limit(1);
...
export async function getModuleBySlug(
  ...
  const dbModule = await db.select().from(modules).where(eq(modules.slug, slug)).limit(1);
...
export async function getRecentlyUpdatedModules(): Promise<Module[]> {
  ...
  const recentReleases = await db
    ...
    .where(and(gte(releases.createdAt, thirtyDaysAgo), eq(modules.isPublished, true)))
    .groupBy(releases.moduleId)
 ...
 */
export async function getTrendingModules(
  options: {
    limit?: number;
...
export async function searchUsers(query: string, roleFilter?: string) {
  let whereClause = or(ilike(user.name, `%${query}%`), ilike(user.email, `%${query}%`));
...
export async function searchUsersAdvanced(
  ...
  if (filters.query && filters.query.trim()) {
    conditions.push(
      or(ilike(user.name, `%${filters.query}%`), ilike(user.email, `%${filters.query}%`)),
    );
  }
  ...
  if (filters.emailVerified !== undefined) {
    const emailVerifiedBool =
      typeof filters.emailVerified === 'string'
  ...
  const users = await (whereClause ? baseQuery.where(whereClause) : baseQuery).orderBy(
    desc(user.createdAt),
  );
  ...
  if (filters.minModules !== undefined || filters.minReviews !== undefined) {
    const usersWithCounts = await Promise.all(
      users.map(async (userRow) => {
        ...
        const meetsModuleReq =
          filters.minModules === undefined || modulesCount >= filters.minModules;
        const meetsReviewReq =
          filters.minReviews === undefined || reviewsCount >= filters.minReviews;
...
export async function getReviewsWithAdvancedFilters(filters: {
  ...
  return await (whereClause ? baseQuery.where(whereClause) : baseQuery).orderBy(
...
export async function getReviewStats() {
  ...
  const avgRatingResult = await db.select({ avgRating: avg(ratings.rating) }).from(ratings);
...
export async function createReply(data: { ratingId: number; userId: string; comment: string }) {
  const result = await db
    .insert(replies)
    .values({
...
export async function checkUserHelpfulVote(
  userId: string,
  ratingId?: number,
  replyId?: number,
): Promise<boolean> {
...
export async function getReleaseSchedule() {
  const result = await db.select().from(releaseSchedule).limit(1);
...
export async function updateModuleGithubSync(
  moduleId: string,
  data: {
    githubRepo?: string;
    enabled?: boolean;
...
export async function getModulesForSync(limit: number = 10) {
  return db
    ...
    .where(and(eq(modules.isPublished, true), eq(moduleGithubSync.enabled, true)))
    .limit(limit);
}
...
export async function getModulesUpdatedThisWeek(): Promise<number> {
  ...
  const result = await db
    ...
    .from(releases)
    .innerJoin(modules, eq(releases.moduleId, modules.id))
    .where(and(gte(releases.createdAt, oneWeekAgo), eq(modules.isPublished, true)))
    .groupBy(releases.moduleId);
...
export async function getNewModulesThisMonth(): Promise<number> {
  ...
  const result = await db
    .select({ count: count() })
    .from(modules)
    .where(and(gte(modules.createdAt, oneMonthAgo), eq(modules.isPublished, true)));
