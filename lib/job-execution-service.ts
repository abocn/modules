...
export class JobExecutionService {
  async executeJob(jobId: number): Promise<void> {
    const [job] = await db.select().from(adminJobs).where(eq(adminJobs.id, jobId));
    ...
    try {
      ...
      const duration = job.startedAt
        ? Math.floor((completedAt.getTime() - new Date(job.startedAt).getTime()) / 1000)
        : 0;
  ...
  private async executeScrapeReleasesJob(
    jobId: number,
    ...
    if (scope === 'all') {
      ...
      modulesToProcess = await db
        ...
        .where(
          and(
            ...
            or(isNull(moduleGithubSync.lastSyncAt), lt(moduleGithubSync.lastSyncAt, cutoffDate)),
          ),
        );
      ...
      modulesToProcess = await db
        ...
        .from(modules)
        .innerJoin(moduleGithubSync, eq(moduleGithubSync.moduleId, modules.id))
        .where(and(eq(modules.id, moduleId), eq(moduleGithubSync.enabled, true)));
    ...
    for (let i = 0; i < modulesToProcess.length; i++) {
      ...
      await this.addJobLog(
        jobId,
        'info',
        `Processing module ${i + 1}/${modulesToProcess.length}: ${module.name}`,
      );
        ...
        if (syncResult.success) {
          ...
          if (syncResult.newReleases > 0) {
            await this.addJobLog(
              jobId,
              'info',
              `✓ ${module.name}: Added ${syncResult.newReleases} new releases`,
            );
  ...
  private async executeCleanupJob(
    ...
    await this.addJobLog(
      jobId,
      'info',
      `Starting cleanup job for ${target}, older than ${days} days`,
    );
    ...
    try {
      if (target === 'failed_jobs') {
        ...
        const deletedJobs = await db
          .delete(adminJobs)
          .where(and(eq(adminJobs.status, 'failed'), lt(adminJobs.createdAt, cutoffDate)))
  ...
  private async executeSyncGithubConfigsJob(jobId: number): Promise<JobExecutionResult> {
    ...
    try {
      const publishedModulesWithGithub = await db
        .select()
        .from(modules)
        .where(and(eq(modules.isPublished, true), isNotNull(modules.sourceUrl)));
      ...
      await this.addJobLog(
        jobId,
        'info',
        `Found ${modulesWithGithubRepos.length} published modules with valid GitHub repos`,
      );

      const existingSyncs = await db.select().from(moduleGithubSync);
        ...
        try {
          if (!existingSyncModuleIds.has(moduleData.id)) {
            ...
            await this.addJobLog(
              jobId,
              'info',
              `✓ Created sync config for ${moduleData.name} (${moduleData.githubRepo})`,
            );
          } else {
            const existingSync = existingSyncs.find((sync) => sync.moduleId === moduleData.id);
            if (
              existingSync &&
              (!existingSync.enabled || existingSync.githubRepo !== moduleData.githubRepo)
            ) {
              await db
                .update(moduleGithubSync)
                .set({
              ...
              await this.addJobLog(
                jobId,
                'info',
                `✓ Updated sync config for ${moduleData.name} (${moduleData.githubRepo})`,
              );
            }
          }
        } catch (error) {
          ...
          await this.addJobLog(
            jobId,
            'error',
            `✗ Failed to sync config for ${moduleData.name}: ${errorMessage}`,
          );
        }
      }
        ...
        if (sync.enabled && !publishedModuleIds.has(sync.moduleId)) {
          try {
            ...
            await this.addJobLog(
              jobId,
              'info',
              `✓ Disabled sync config for unpublished module ${sync.moduleId}`,
            );
          } catch (error) {
            errors++;
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            ...
            await this.addJobLog(
              jobId,
              'error',
              `✗ Failed to disable sync config for ${sync.moduleId}: ${errorMessage}`,
            );
          }
        }
      }
  ...
  private async executeGenerateSlugsJob(jobId: number): Promise<JobExecutionResult> {
    ...
    try {
      ...
      await this.addJobLog(
        jobId,
        'info',
        `Found ${modulesWithoutSlugs.length} modules without slugs`,
      );
        ...
        try {
          ...
          if (processedCount % 10 === 0) {
            await this.updateJobProgress(
              jobId,
              Math.floor((processedCount / modulesWithoutSlugs.length) * 100),
            );
            await this.addJobLog(
              jobId,
              'info',
              `Processed ${processedCount}/${modulesWithoutSlugs.length} modules`,
            );
          }
        } catch (error) {
          errorCount++;
  ...
  private async updateJobStatus(
    jobId: number,
    status: string,
    updates: Partial<typeof adminJobs.$inferInsert> = {},
  ) {
    await db
      .update(adminJobs)
      .set({
  ...
  async startPendingJobs(): Promise<void> {
    const pendingJobs = await db.select().from(adminJobs).where(eq(adminJobs.status, 'pending'));
