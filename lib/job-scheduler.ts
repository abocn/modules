...
export class JobScheduler {
  ...
  async start() {
    ...
    console.log(
      `[Job Scheduler] Scrape interval: ${this.scrapeInterval}ms (${this.scrapeInterval / 1000 / 60 / 60} hours)`,
    );
  ...
  private async scheduleAutomaticScrape() {
    ...
    try {
      ...
      const existingJobs = await db.select().from(adminJobs).where(eq(adminJobs.status, 'pending'));

      const hasPendingScrape = existingJobs.some(
        (job) => job.type === 'scrape_releases' && job.name === 'Automatic GitHub Scrape',
      );
