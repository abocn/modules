...
export class GitHubService {
  ...
  async getReleases(owner: string, repo: string, perPage = 10): Promise<GitHubRelease[]> {
    try {
      ...
      console.log(
        `[GitHub Service] Successfully fetched ${data.length} releases from ${owner}/${repo}`,
      );
  ...
  async syncModuleReleases(
    ...
    try {
      ...
      const existingGitHubIds = new Set(
        existingReleases.map((r) => r.githubReleaseId).filter((id) => id !== null),
      );

      console.log(
        `[GitHub Service] Found ${githubReleases.length} total releases, ${existingGitHubIds.size} already synced`,
      );
      ...
      for (const githubRelease of githubReleases) {
        ...
        try {
          ...
          console.log(
            `[GitHub Service] Prepared release ${githubRelease.tag_name} for module ${moduleId}`,
          );
        } catch (error) {
          const message = error instanceof Error ? error.message : 'Unknown error';
          console.error(
            `[GitHub Service] Failed to prepare release ${githubRelease.tag_name}:`,
            message,
          );
      ...
      if (newReleases.length > 0) {
        ...
        await db.update(releases).set({ isLatest: false }).where(eq(releases.moduleId, moduleId));
        ...
        for (const releaseData of newReleases) {
          try {
            ...
            result.newReleases++;
            console.log(
              `[GitHub Service] Successfully added release ${releaseData.githubTagName} for module ${moduleId}${releaseData.isLatest ? ' (marked as latest)' : ''}`,
            );
          } catch (error) {
            const message = error instanceof Error ? error.message : 'Unknown error';
            console.error(
              `[GitHub Service] Failed to insert release ${releaseData.githubTagName}:`,
              message,
            );
        ...
        if (!latestNewRelease && latestVersion) {
          await db
            .update(releases)
            .set({ isLatest: true })
            .where(and(eq(releases.moduleId, moduleId), eq(releases.version, latestVersion)));
        }
      }
      ...
      if (result.newReleases > 0) {
        ...
        await db.update(modules).set({ lastSyncAt: new Date() }).where(eq(modules.id, moduleId));
      }

      result.success = result.errors.length === 0;

      console.log(
        `[GitHub Service] Sync completed for module ${moduleId}: ${result.newReleases} new releases, ${result.errors.length} errors`,
      );
  ...
  async validateRepository(owner: string, repo: string): Promise<boolean> {
    try {
      ...
      console.error(
        `[GitHub Service] Repository ${owner}/${repo} validation failed:`,
